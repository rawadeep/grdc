...
class SettingController extends Controller
{
    ...
    public function change(Request $request)
    {
        ...
        if (isset($data['footer_links'])) {
            ...
            $footerLinksSetting = \App\Models\Tamang\Setting::where('key', 'footer_links')->first();
            if ($footerLinksSetting) {
                $footerLinksSetting->update(['value' => json_encode($links)]);
            } else {
                \App\Models\Tamang\Setting::create([
                    'name' => 'Footer Links',
                    'key' => 'footer_links',
            ...
            for ($i = 1; $i <= 10; $i++) {
                \App\Models\Tamang\Setting::where('key', 'link_url_' . $i)->delete();
                \App\Models\Tamang\Setting::where('key', 'link_title_' . $i)->delete();
            }
        }
        foreach ($data as $key => $value) {
            \App\Models\Tamang\Setting::where('key', $key)->update(['value' => $value]);
        }
