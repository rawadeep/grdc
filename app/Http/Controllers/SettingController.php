...
class SettingController extends Controller
{
    ...
    public function change(Request $request)
    {
        ...
        if (isset($data['footer_links'])) {
            ...
            for ($i = 1; $i <= 10; $i++) {
                \App\Models\Admin\Setting::where('key', 'link_url_' . $i)->delete();
                \App\Models\Admin\Setting::where('key', 'link_title_' . $i)->delete();
            }
        }
        // Only update known settings keys (excluding footer_links)
        $existingKeys = \App\Models\Admin\Setting::pluck('key')->toArray();
        foreach ($data as $key => $value) {
            if ($key !== 'footer_links') {
                $existing = \App\Models\Admin\Setting::where('key', $key)->first();
                \App\Models\Admin\Setting::updateOrCreate(
                    ['key' => $key],
                    [
                        ...
                        'value' => $value,
                        'type' => 'text',
                        'group' => $existing ? $existing->group : 0,
                    ]
                );
            }
        }
