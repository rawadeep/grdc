<x-layouts.backend>
    <x-backend.card>
        ...
        <form action="{{ route(prefixed_route('project-descriptions.update'), $projectDescription->id) }}"
            method="post">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-10">
                    <div class="form-group">
                        ...
                        <x-backend.input.default name="title" required="required"
                            placeholder="{{ get_phrase('Title') }}" value="{{ $projectDescription->title }}" />
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <x-backend.input.default type="number" min="1" name="order" required="required"
                            placeholder="{{ get_phrase('Order') }}" value="{{ $projectDescription->ord }}" />
                    </div>
                </div>
            </div>

            <x-backend.input.icon-picker value="{{ $projectDescription->icon }}" />
